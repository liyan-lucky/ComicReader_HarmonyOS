...
- 设置功能和底部“设置”Tab 应保留。
- 搜索页顶部包含语言切换和主题按钮，属于设置类入口。
- 搜索框下方包含搜索模式、来源过滤、当前搜索引擎、源数量状态，属于设置/配置展示。
- 搜索框下方还有“纯封面/封面+名称”切换和“演示”按钮，不属于核心搜索功能。
- 搜索首页里的“去设置引擎”按钮会直接跳到设置页，主页面看起来比较怪。

建议处理：

- 保留底部 Tab：搜索 / 书架 / 设置 / 关于。
- 保留 `SettingsPage()` 和 `activeTab === 'settings'` 的页面逻辑。
- 搜索页只保留：标题、搜索输入框、搜索按钮、搜索状态、搜索结果、最近阅读。
- 删除搜索页里的语言/主题入口、搜索模式状态条、纯封面切换、演示按钮、去设置引擎按钮。
