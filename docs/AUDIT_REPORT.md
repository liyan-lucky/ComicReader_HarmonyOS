...
## 2026-09-20 本轮代码审计（v0.88.5）

### 审计范围
对 v0.87.44 → v0.88.5 变更进行代码清理与死代码审计。
...
|---|---|---|
| 未使用 @State 变量 | 6 个 | 删除 |
| 未使用 private 变量 | 1 个 | 删除 |
| 未使用 private 方法 | 16 个 | 删除 |
| 未使用 @Builder 方法 | 8 个 | 删除 |
| 未使用导入 | 2 个 | 删除 |
| **合计删除** | **310 行** | 无功能影响 |

### SearchEngines.ets 性能优化

**问题**：`containsNoiseTitleWord` 方法对标题做三次独立遍历匹配噪声词。

**优化**：合并三次遍历为一次遍历，同时移除死参数 `strict`（调用方均未依赖该参数的分支行为）。

**效果**：噪声词匹配由 O(3n) 降为 O(n)，减少字符串遍历开销。

### 无用文件清理

| 删除对象 | 说明 |
|---|---|
| `nul` | 垃圾文件（Windows 保留名误生成） |
| `.tmp/` 目录 | 临时目录，含 66 个文件 |
| `migration/` 目录 | 迁移残留目录 |

### 审计结论
本轮清理纯死代码与无用文件，无功能变更。构建验证通过（v0.88.5，0 ERROR）。
