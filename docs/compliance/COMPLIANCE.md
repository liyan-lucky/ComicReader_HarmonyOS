...
本仓库是 HarmonyOS ArkTS Stage 漫画浏览器 App 主仓库。公开源规则拆分到：

```text
liyan-lucky/ComicReader_Rules
```

App 主仓库不托管、不上传、不分发漫画图片、章节正文、付费内容、账号数据、站点 Logo、字体、SDK 压缩包、HAP/APP 发布包或其他第三方受保护资源。
...
- `build-profile.json5` 当前无签名配置；
- `module.json5` 当前仅申请 `ohos.permission.INTERNET`；
- 未发现明显硬编码密码、Token、Cookie、私钥、签名证书或账号数据；
- 未发现明显打包漫画图片、章节正文、字体、Logo、HAP/APP 发布包、SDK 压缩包或来源不明二进制资源；
...
- `SearchEngines.ets`：过滤抖音、短视频、百科、社区、购物等非漫画结果。

## CI 兼容边界

CI 兼容脚本只服务于命令行 SDK 构建验证，不改变本地 DevEco Studio 常规开发流程。
